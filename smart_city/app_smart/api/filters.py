...
class SensorFilterView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        filters = Q()
        if tipo: filters &= Q(tipo__icontains=tipo)  
        if localizacao: filters &= Q(localizacao__icontains=localizacao)  
        if responsavel: filters &= Q(responsavel__icontains=responsavel) 
...
class TemperaturaFilterView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        filters = Q()
        if sensor_id:
            filters &= Q(sensor__id=sensor_id)
        if valor_gte:

...
class SensorFilterView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        filters = Q()
        if tipo: filters &= Q(tipo__icontains=tipo)  # Corrigido aqui
        if localizacao: filters &= Q(localizacao__icontains=localizacao)  # Corrigido aqui
        if responsavel: filters &= Q(responsavel__icontains=responsavel)  # Corrigido aqui
        if status_operacional is not None: filters &= Q(status_operacional=status_operacional)
...
class TemperaturaDataFilter(django_filters.FilterSet):
    ...
    class Meta:
        ...
        fields = ['timestamp_gte', 'timestamp_lte', 'sensor', 'valor gte', 'valor lte']
